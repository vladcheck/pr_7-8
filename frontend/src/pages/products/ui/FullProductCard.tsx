...
const FullProductCard = observer(
	({
		...
		return (
			<FlexContainer
				flexDir="col"
				className="gap-6 glass-panel p-6 max-w-[900px] w-full mx-auto my-8 relative"
			>
				{canEdit && (
					<FlexContainer className="absolute top-4 right-4 gap-2 z-10 transition-all">
						<Button
							onClick={() => navigate(`/products/edit/${p.id}`)}
							className="bg-primary/10 hover:bg-primary/20 text-primary px-4 py-2 rounded-xl border border-primary/20 transition-all font-medium backdrop-blur-sm"
						>
							Изменить
						</Button>
						<Button
							onClick={handleDelete}
							className="bg-red-500/10 hover:bg-red-500/20 text-red-500 px-4 py-2 rounded-xl border border-red-500/20 transition-all font-medium backdrop-blur-sm"
						>
							Удалить
						</Button>
					</FlexContainer>
